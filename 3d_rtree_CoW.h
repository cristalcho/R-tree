		...
		int dataID, struct Node *n, struct Node **new_node, struct Node *p, struct splitLog *sl)
{
	...
	//
  MetaData<META> mstore;
  MetaData<META> cstore;
  memcpy(&mstore,&n->meta, sizeof(MetaData<META>));
  bool sp = false;
	if (!n->meta.IsLeaf())
	{
#ifdef SHARED
...
#else
    {
      ...
      gettimeofday(&tr1,0); // start the stopwatch
#endif
      pthread_mutex_lock(n->branch[i].child->mut);
      memcpy(&cstore, &n->branch[i].child->meta, sizeof(MetaData<META>));
      if((n->branch[i].child->meta.IsLeaf()&&!n->branch[i].child->meta.SplitCheck())){
//          (!n->branch[i].child->meta.IsLeaf()&&!n->branch[i].child->meta.IsFull())){
        unlocked = true;
        ...
        rt = RTreeAddBranch(&b2, n, new_node, p, log);
          if(rt == 1){
              ...
              if(!n->meta.Bit(i))
                ...
                if(pthread_mutex_trylock(n->mut) == 0){
                  printf("REALLY\n");
                }
                if(!n->meta.Bit(i))
                  SJ();
...
#endif
  while(1){
    pthread_mutex_lock(my_root->mut);
    if(my_root == *root){
      break;
    }
    pthread_mutex_unlock(my_root->mut);
    my_root = *root;
  }
